...
def build_ai_quote_result(
    ...
    # 服务端 result schema 要求 completeness_score / classification_confidence 为 number：
    # AI 报价模式不做完整度/分类评估，置占位值（不触发服务端相应审核，与原型行为一致）。
    # 注意 classification_confidence 服务端列为 decimal(4,4)，最大值 0.9999，不能填 1.0。
    completeness_placeholder = 1.0
    confidence_placeholder = 0.99
    snapshot["completeness_score"] = completeness_placeholder
    # 兼容服务端二次审核可能读取的 classification 字段（与 QRS 快照结构对齐）
    ...
    return {
        ...
        "result": {
            ...
            "completeness_score": completeness_placeholder,
            "classification_confidence": confidence_placeholder,

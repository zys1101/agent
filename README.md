...
- 与 AI_quote 原型一致：**不评估需求完整度、不做项目分类、不因信息不足拒报，始终输出价格**（`completeness_score`/`classification_confidence` 为占位值 1.0，`missing_information`/`clarification_questions` 为空数组）。
